...
```
eksctl create cluster --name AIMricrosevice-cluster --region ap-south-1 --node-type t2.medium  --zones ap-south-1a,ap-south-1b
```
...
pipeline {
    ...
    stages {
        stage('Clone Repo') {
            steps {
                git 'https://github.com/jhaavilash/AI-Microservices-App-Kubernetes--Jenkins-CI-CD.git'
            }
        }
        stage('Maven Build') {
            steps {
        ...
        stage('Docker Image') {
            steps {
                sh 'docker build -t Avilash/AI-Microservices-App-Kubernetes .'
            }
        }
...
## Noted :-  After tested, delete the Cluster and other resources 
